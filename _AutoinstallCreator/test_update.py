...
class TestUpdate(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        pathlib.Path(os.path.join(_self_path, "AutoinstallCreator.sh")).unlink(
            missing_ok=True
        )
        pathlib.Path(os.path.join(_self_path, "AutoinstallCreator.sh.bat")).unlink(
            missing_ok=True
        )
        ...
        subprocess.run(
            busybox_exe_path_arg + [os.path.join(_self_path, "release.sh")], check=True
        )
    ...
    def setUp(self):
        ...
        self.old_update_log_filepath = os.path.join(
            _self_tmp_path, self.test_old_version, "_update.log"
        )
        self.update_log_filepath = os.path.join(
            _self_tmp_path, self.version_str, "_update.log"
        )
    ...
    def test_version_up_to_date(self):
        subprocess.run(
            busybox_exe_path_arg + [self.package_filepath],
            cwd=_self_tmp_path,
            check=True,
        )

        subprocess.run(
            [os.path.join(_self_tmp_path, self.version_str, update_script_name)],
            env={
                **os.environ,
        ...
        self.assertTrue(
            version_is_up_to_date(self.update_log_filepath, self.version_str)
        )

    @unittest.skipIf(
        sys.platform.startswith("win")
        and not os.path.exists(r"C:\Windows\notepad.exe"),
        "disable in nanoserver",
    )
    def test_update_to_new_version(self):
        subprocess.run(
            busybox_exe_path_arg
            + [self.package_filepath, "--target", self.test_old_version],
            cwd=_self_tmp_path,
            check=True,
        )
        ...
        subprocess.run(
            [os.path.join(_self_tmp_path, self.test_old_version, update_script_name)],
            env={
                **os.environ,
                "MOCK_AUTOINSTALLCREATOR_VERSION_BODY": self.version_str,
                "MOCK_AUTOINSTALLCREATOR_PACKAGE_FILEPATH": self.package_filepath,
        ...
        self.assertTrue(
            io_tools.wait_for(
                lambda: update_completed(
                    self.old_update_log_filepath, self.version_str
                ),
